...
async function connectAccount(id, phone, proxyRaw = null) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            addLog('warn', `Account ${phone} disconnected: ${reason}`);
            ...
            if (statusCode !== DisconnectReason.loggedOut) {
                setTimeout(() => {
                    if (accounts[id]) {
                        addLog('info', `Attempting to reconnect ${phone}...`);
                        connectAccount(id, phone, proxyRaw);
                    }
                }, 5000);
            } else {
                addLog('danger', `Account ${phone} logged out. Remove and re-add.`);
    ...
    if (!sock.authState.creds.registered) {
        try {
            ...
            accounts[id].pairingCode = code;
            addLog('info', `Pairing code for ${phone}: ${code}`);
            return { success: true, pairingCode: code };
        } catch (err) {
            addLog('danger', `Failed to get pairing code for ${phone}: ${err.message}`);
...
app.post('/api/accounts', async (req, res) => {
    ...
    const availableProxy = proxies.find(p => p.status === 'alive' && !Object.values(accounts).some(a => a.proxy === p.raw));
    const proxyRaw = availableProxy ? availableProxy.raw : null;
    
    addLog('info', `Adding account ${cleanPhone}...`);
    
    const result = await connectAccount(id, cleanPhone, proxyRaw);
...
app.post('/api/proxies', (req, res) => {
    ...
    proxies = lines.map((raw, i) => {
        const parsed = parseProxy(raw);
        return {
            id: i,
            raw: raw.trim(),
            status: parsed ? 'alive' : 'dead',
            ...parsed
        };
    });
    
    addLog('info', `Loaded ${proxies.length} proxies`);
